...
public class Reproduction extends CustomCard {

    public static final String ID = IdHelper.makePath("Reproduction");
    private static final CardStrings CARD_STRINGS = CardCrawlGame.languagePack.getCardStrings(ID); // 从游戏系统读取本地化资源
    private static final String NAME = CARD_STRINGS.NAME;
    private static final String IMG_PATH = ImageHelper.getCardImgPath(CardType.SKILL, "Reproduction");
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        if(!p.hasPower(Bio.POWER_ID)){
             this.addToBot(new ApplyPowerAction(p, p, new Bio(p, 1), 1));
        }
    }
