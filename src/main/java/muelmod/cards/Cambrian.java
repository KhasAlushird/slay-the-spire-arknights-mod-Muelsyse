...
import muelmod.powers.CambrianPower;
import muelmod.powers.Fluid;

public class Cambrian extends CustomCard {

    public static final String ID = IdHelper.makePath("Cambrian");
    private static final CardStrings CARD_STRINGS = CardCrawlGame.languagePack.getCardStrings(ID); // 从游戏系统读取本地化资源
    private static final String NAME = CARD_STRINGS.NAME;
    private static final String IMG_PATH = ImageHelper.getCardImgPath(CardType.SKILL, "Cambrian");
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        if (p.hasPower(Fluid.POWER_ID)){
            ...
            if (curr_fluid_amount<=this.magicNumber){
                ...
                this.addToBot(new ApplyPowerAction(p, p, new CambrianPower(p,3 ,curr_fluid_amount),curr_fluid_amount));
            }
            else{
                this.addToBot(new com.megacrit.cardcrawl.actions.common.ReducePowerAction(p, p, Fluid.POWER_ID, this.magicNumber));
                this.addToBot(new ApplyPowerAction(p, p, new CambrianPower(p,3 ,this.magicNumber), this.magicNumber));
            }

        }
        else{
            this.addToBot(new ApplyPowerAction(p, p, new CambrianPower(p,3 ,0),0));
        }
